  ...
  <div class="min-h-screen bg-[#f9f5ef] font-serif">
    <main class="container mx-auto px-4 py-8">
      ...
      <div v-else class="grid grid-cols-1 gap-8 lg:grid-cols-5">
        ...
        >
          <!-- The old notification div has been removed from here -->
          <RecipeForm
            @submit="handleUpdate"
...
// --- Fetching existing recipe data ---
const { result, loading, error } = useQuery(
  GET_RECIPE_DETAILS,
  { id: recipeId, user_id: currentUserId.value },
  { enabled: !!currentUserId.value, fetchPolicy: "cache-and-network" }
);

const recipeData = computed<RecipeDetail | null>(() => {
  // Add a security check: ensure the editor is the owner
...
// --- Preparing data for the form component ---
...
// Helper functions to resolve categories and ingredients (unchanged)
...
async function handleUpdate({
  ...
  try {
    ...
    // 3. Use a success toast for positive feedback
    ...
    // 4. Use an error toast for failures
      ...
      <div v-else class="grid grid-cols-1 gap-8 lg:grid-cols-5">
        ...
        >
          <div
            v-if="notification"
            :class="[
              'relative mb-6 rounded-lg border px-4 py-3',
          ...
          >
            ...
            <span class="ml-2 block sm:inline">{{ notification.message }}</span>
            <button
